...
```bash
git tag --sort=-v:refname | head -1     # latest tag, e.g. v5.1.0
cfo -v                                   # version baked into the built binary
```

The version string in the repo currently defaults to `5.1.0` — the latest
...
```powershell
Write-Host '  -Version    Overrides the version string (default: 5.1.0).'  # ← update
```
...
```powershell
pwsh -File .\build-all.ps1 -Version 5.2.0
```

```bash
VERSION=5.2.0 ./build-all.sh
```
...
2. **Cross-compiles 8 targets** in parallel, all `CGO_ENABLED=0` (fully
   ...
   `freebsd/amd64`.
3. Writes SHA256 sums to `dist/checksums.txt`.
4. **UPX**-compresses copies into `dist/compressed/` (if `upx` present).
5. **Unix only** — writes a source archive `dist/cipherforge_source.tar.gz`.
...
```
release-assets/
├── cfo_v5.2.0_linux_amd64.zip
├── cfo_v5.2.0_linux_arm64.zip
├── cfo_v5.2.0_linux_386.zip
├── cfo_v5.2.0_windows_amd64.zip
├── cfo_v5.2.0_windows_386.zip
├── cfo_v5.2.0_darwin_amd64.zip
├── cfo_v5.2.0_darwin_arm64.zip
├── cfo_v5.2.0_freebsd_amd64.zip
└── checksums.txt
```

> Optionally also upload the source archive produced by the Unix build
> (`dist/cipherforge_source.tar.gz`) as `source-v5.2.0.tar.gz`. Note that
...
```bash
git commit -m "Release v5.2.0: <short summary of what's new>"
```
...
```bash
git tag -a v5.2.0 -m "Cipherforge v5.2.0"
```
...
git push origin v5.2.0          # or: git push origin --tags
```
...
```bash
gh release create v5.2.0 --title "Cipherforge v5.2.0" --notes-file ../release-notes.md
gh release upload v5.2.0 cfo_v5.2.0_*.zip checksums.txt
```
...
1. **Binary version stamp** — download and run on the platforms you can test:
   ...
   ```bash
   curl -sSL -o cfo https://github.com/vilshansen/cipherforge-go/releases/download/v5.2.0/cfo_v5.2.0_linux_amd64.zip
   unzip -p cfo_v5.2.0_linux_amd64.zip cfo > cfo
   chmod +x cfo && ./cfo -v
   ```
...
2. **Checksums** — confirm the uploaded `checksums.txt` matches what you
   ...
   ```bash
   curl -sSL https://github.com/vilshansen/cipherforge-go/releases/download/v5.2.0/checksums.txt
   ```
...
- [ ] `./build-all.sh` (or `build-all.ps1`) passes: tests green, all 8
      targets compiled, `dist/checksums.txt` written.
