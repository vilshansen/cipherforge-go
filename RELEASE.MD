...
```bash
git tag --sort=-v:refname | head -1     # latest tag, e.g. v7.2.1
cfo -v                                   # version baked into the built binary
```

The version string in the repo currently defaults to `7.2.1` — the latest
...
if (-not $Version) {
    $Version = '7.2.1'            # ← change this
}
```

```powershell
Write-Host '  -Version    Overrides the version string (default: 7.2.1).'  # ← update
```
...
```powershell
pwsh -File .\build-all.ps1 -Version 7.2.1
```

```bash
VERSION=7.2.1 ./build-all.sh
```
...
```
release-assets/
├── cfo_v7.2.1_linux_amd64.zip
├── cfo_v7.2.1_linux_arm64.zip
├── cfo_v7.2.1_linux_386.zip
├── cfo_v7.2.1_windows_amd64.zip
├── cfo_v7.2.1_windows_386.zip
├── cfo_v7.2.1_darwin_amd64.zip
├── cfo_v7.2.1_darwin_arm64.zip
├── cfo_v7.2.1_freebsd_amd64.zip
└── checksums.txt
```

> Optionally also upload the source archive produced by the Unix build
> (`dist/cipherforge_source.tar.gz`) as `source-v7.2.1.tar.gz`. Note that
...
```bash
git commit -m "Release v7.2.1: <short summary of what's new>"
```
...
```bash
git tag -a v7.2.1 -m "Cipherforge v7.2.1"
```
...
git push origin v7.2.1          # or: git push origin --tags
```
...
```bash
gh release create v7.2.1 --title "Cipherforge v7.2.1" --notes-file ../release-notes.md
gh release upload v7.2.1 cfo_v7.2.1_*.zip checksums.txt
```
...
1. **Binary version stamp** — download and run on the platforms you can test:
   ...
   ```bash
   curl -sSL -o cfo https://github.com/vilshansen/cipherforge-go/releases/download/v7.2.1/cfo_v7.2.1_linux_amd64.zip
   unzip -p cfo_v7.2.1_linux_amd64.zip cfo > cfo
   chmod +x cfo && ./cfo -v
   ```
...
2. **Checksums** — confirm the uploaded `checksums.txt` matches what you
   ...
   ```bash
   curl -sSL https://github.com/vilshansen/cipherforge-go/releases/download/v7.2.1/checksums.txt
   ```
