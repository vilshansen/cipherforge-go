...
import (
	"bytes"
	"crypto/rand" // Standard sikker kilde til tilfældighed
	"encoding/binary"
	"fmt"
	"io"
	"log"
	"math/big" // Nødvendig for crypto/rand.Int til at undgå modulo bias
	"os"
	"path/filepath"
	"syscall"
...
// generateSecurePassword genererer et sikkert, tilfældigt password UDEN modulo bias.
func generateSecurePassword(length int) string {
	poolLen := big.NewInt(int64(len(CharacterPool)))
	result := make([]byte, length)

	for i := 0; i < length; i++ {
		// crypto/rand.Int sikrer, at det genererede tal i intervallet [0, poolLen)
