...
- **Generated secrets are now 45 characters, printed in dash-separated groups of
  five.** 45 is the smallest multiple of five whose keyspace still clears 256
  bits (~262 bits of entropy), and the grouping makes a long secret far easier
  to read aloud or transcribe without losing your place. The separators are part
  of the secret, so it is entered exactly as displayed.
- **Secrets generated by earlier releases keep working.** The file format never
  depended on the secret's length — any secret still derives the same keys — so
  existing `.cfo` files decrypt with the secret you saved.
